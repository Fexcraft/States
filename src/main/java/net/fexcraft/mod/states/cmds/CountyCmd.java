...
public class CountyCmd extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if(args.length == 0){
			Print.chat(sender, "&7/ct info");
			Print.chat(sender, "&7/ct rules");
			Print.chat(sender, "&7/ct join");
			Print.chat(sender, "&7/ct leave");
			Print.chat(sender, "&7/ct kick <player> <reason>");
			Print.chat(sender, "&7/ct invite <player> <msg>");
			if(mun.manage.isInCouncil(ply.getUUID()) || StateUtil.isAdmin(player)){
				Print.chat(sender, "&8- &5- &8- - - - - -");
				Print.chat(sender, "&7/ct vote-manager <player>");
				Print.chat(sender, "&7/ct leave-council");
			}
			Print.chat(sender, "&8- &6- &8- - - - - -");
			Print.chat(sender, "&7/ct create <name...>");
			Print.chat(sender, "&7/ct buy");
			return;
		}
		switch(args[0]){
			case "info":{
			...
			case "buy":{
				if(true){//TODO permission for this will be stored in state ruleset //hasPerm("municipality.buy", player, mun)){
					...
					//
					County playercounty = ply.getCounty();
			...
			case "create":{
				...
				if(!ply.canLeave(sender, false)) return;
